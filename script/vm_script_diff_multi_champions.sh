#!/bin/bash
	...
	if [ $# -lt 5 -o $# -gt 8 ]; then
	echo -e "Usage : sh script/vm_script_diff_multi_champions.sh <nb players> <cycle start> <cycle end> <dump slices> <player(s) path>";
		exit
	elif [ $1 -lt 1 -o $1 -gt 4 ]; then
	echo -e "Error : Number of player must be between 1 and 4";
		exit;
	elif [ ! -f "corewar" ]; then
		make;
	fi
